...
def get_predictions_from_mongo(user_id):
    client = pymongo.MongoClient("mongodb+srv://qh2023:1@clusterbegin0.iojcaoc.mongodb.net") 
    db = client["film_application"]
    collection = db["predictions"]
    ...
    # If the user document exists, update it with the new movie and prediction
    if user_document:
        movies = user_document.get("movies", [])[0]
        movie_ids = movies.get("movie_id", [])
        predictions = movies.get("prediction", [])

    # Close the MongoDB connection
    client.close()
    return {'user_id': user_id, 'movies': {'movie_id': movie_ids, 'predictions': predictions}}
...
def check_user_id_availability(user_id: int):
    ...
    if not user_document:
        raise HTTPException(status_code=404, detail="Máy yếu quá em chưa kịp làm cho user này ạ 🥲. Anh chị thử chỉnh retrain=True xem sao.")
    client.close()
